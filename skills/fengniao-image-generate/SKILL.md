...
2. `transform`、`expand` 和 `enhance` 必须有源图，最多支持六张参考图；多图按“身份主图 → 本张需要的角度/结构/细节 → 风格参考”的顺序提供。电商任务默认只传身份主图和零至两张直接相关的证据图，不把全部素材无差别传给每张。本地图片由 CLI 自动直传，不要求用户处理上传步骤或重复描述任务。
3. 多张参考图可使用 `reference_images` 数组，也兼容 `image` 数组和 `reference_image_urls`；不要混用本地文件与 HTTPS URL。第一张作为 `original` 比例基准，其余图片的使用方式由参考模式和职责决定。
4. 同一人物或商品的多角度、结构和细节补充使用默认 `reference_mode=single_subject`，未写职责时才按“第一张身份主图、其余同主体补充图”处理。通用设计中出现人物图、商品图、构图图或风格图等彼此独立的参考来源时，必须使用 `reference_mode=composition`，并传入与图片数量一致的 `reference_roles`，例如 `["人物主体：保持面部和服装", "商品主体：保持包装外观", "风格参考：只采用色彩和光线"]`；不能套用商品身份锁、SKU 或同主体规则。
5. 电商多图的职责写成 `["main-005｜商品身份主图", "main-003｜本张尺寸与材质证据"]` 这类“稳定文件 ID｜职责”。CLI 会把职责按编号写入 Prompt，同时保持上传后的 key 顺序不变；任务 Prompt 使用“参考图 1（main-005）”引用，不能只写文件名。多个颜色或 SKU 同框比较时使用 `reference_mode=variants`，每个变体提供一张身份图，并在职责中写明颜色/SKU；此模式下各参考图并列，不能用参考图 1 覆盖其他颜色。
6. 只有目标版式不明确时才追问比例。文生图默认 `1:1`，保持源图的改图/增强默认 `original`。
7. 不要求普通用户选择模型别名；没有明确模型要求时统一使用 `tpro-1k`：
...
8. 每次请求固定生成一张；多张结果必须拆成多个新 `request_id` 的请求。多任务执行前建立清单，记录“任务编号、用途、Prompt 版本、参考图顺序、比例、request_id、结果路径”；只重做指定任务，其他任务不重复提交。
...
共享同一组商品参考图的电商套图从 2 张起必须使用持久化批量 action；普通独立任务达到 11 张时必须使用。不能由 Agent 临时编写 Shell 脚本、循环调用或并行启动多个单图命令替代批量 action：
...
9. 批量生成与视觉检查解耦。任务成功只依据批次 manifest 中的终态和产物落盘，不等待图片读取工具才继续补入生成队列。若当前 Agent 的图片读取工具明确支持单次超时、取消和终态回收，视觉检查最多 2 张并发，并为每张设置不超过 60 秒的工具超时；缺少其中任一能力时不自动调用视觉读取，先交付批次汇总、结果目录和已完成文件，并把检查状态记为“等待商家确认”。用户在交付后明确要求 AI 检查时才一次读取 1 张，仍不得并行。大批量不在对话铺开数百条路径。任何图片读取失败、超时或不可用都不能将已完成生图改判为失败，也不能阻塞剩余生成、结果展示或批次恢复。
...
直接展示返回的图片；展示文件不等于把多张图片同时送入视觉模型读取。用户要求像素级一致时，明确扩图和增强属于生成式处理。遇到 `CREDITS_INSUFFICIENT` 时保留输入，并引导用户登录 https://fengniaoai.com/ 获取点数。
