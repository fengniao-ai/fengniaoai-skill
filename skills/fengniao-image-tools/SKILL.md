...
description: >-
  蜂鸟AI 生图的修图能力。用户明确要求抠图、去背景、移除背景或抠出主体时，可以自动识别
  人像、商品、服装、Logo 或宠物，并输出指定的透明底、白底或主体裁切图；也可以提取图片
  中的文字，并返回每段文字的位置坐标。当用户明确说抠图、去背景、移除背景、抠出主体，
  或要求识别图片文字、提取文案、OCR、查找文字坐标时使用。仅说“白底图”不能触发抠图。
---
...
必须提供一张图片，并且用户必须明确表达抠图或去除背景意图。仅说“做成白底图”“白底主图”“换成白色背景”时不要调用本能力，改用 `fengniao-image-generate` 的 `image transform`。明确抠图后，将用户意图映射为稳定的公开参数：
