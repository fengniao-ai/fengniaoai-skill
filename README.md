...
从 `1.4.x` 或更早版本升级时，如果客户端仍显示多个旧子条目，说明安装目录残留了旧版 `skills/*/SKILL.md`。请先通过该 Agent 平台的 Skill 管理功能移除旧安装，再从 GitHub 干净安装 `1.5.1` 或更高版本，并重新打开客户端或开始新对话。不同平台的卸载命令并不统一，因此不要直接删除不确定的用户目录。
